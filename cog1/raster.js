    ...
    function (exports, shader, framebuffer, data) {
        ...
        function drawLineBresenham(startX, startY, startZ, endX, endY, endZ, color, storeIntersectionForScanlineFill, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord) {
            ...
            if (dXAbs >= dYAbs) {
                ...
                deltaInterpolationWeight = 1.0 / (dXAbs === 0 ? 1 : dXAbs)
                ...
                deltaInterpolationWeight = 1.0 / (dYAbs === 0 ? 1 : dYAbs)
        ...
        function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
            ...
            do {
                startPoint = vertices[polygon[lastIndex]];

                let nextVertexIndex = (lastIndex > polygon.length) ? lastIndex - 1 : 0;
                endPoint = vertices[polygon[nextVertexIndex]];

                lastDerivative = calcDerivative(Math.floor(startPoint[1]), Math.floor(endPoint[1]))
                lastIndex--;
            } while (lastIndex >= 0 && (lastDerivative === 0 || lastDerivative === undefined))
            ...
            // Loop over vertices/edges in polygon.
            // console.log(`${polygon.length} punkte gefunden`)
            for (let v = 0; v < polygon.length; v++) {
                // Determine start and end point of edge.
                startPoint = polygon[v];

                // Connect edge to next or to first vertex to close the polygon.
                let nextVertexIndex = (v < polygon.length - 1) ? v + 1 : 0;
                endPoint = polygon[nextVertexIndex];


                // Convert parameters to integer values.
                // Use Math.floor() for integer cast and rounding of X-Y values.
                // Leave Z as floating point for comparisons in z-buffer.
                let curr = vertices[startPoint]
                currX = Math.floor(curr[0])
                currY = Math.floor(curr[1])
                currZ = curr[2]

                let next = vertices[endPoint]
                nextX = Math.floor(next[0])
                nextY = Math.floor(next[1])
                nextZ = next[2]

                // Set texture coordinate uv-vector/array of the current edge for later interpolation.
                let edgeStartTextureCoord, edgeEndTextureCoord;

                if (texture) {
                    edgeStartTextureCoord = textureCoord[polygonTextureCoord[v]]
                    edgeEndTextureCoord = textureCoord[polygonTextureCoord[nextVertexIndex]]
                }
                ...
                drawLineBresenham(currX, currY, currZ, nextX, nextY, nextZ, color, true, startPoint, endPoint, edgeStartTextureCoord, edgeEndTextureCoord);


                // Add end point of non horizontal edges.
                // addIntersection(nextX, nextY, nextZ, 1, startPoint, endPoint, edgeStartTextureCoord, edgeEndTextureCoord)
                addIntersection(nextX, nextY, nextZ, 1, startPoint, endPoint, edgeStartTextureCoord, edgeEndTextureCoord)

                // Last derivative has to exist, always, also for the first edge.
                // if (lastDerivative === undefined) continue;

                // Add start point if edges are non monotonous, Peek point.
                if (derivative + lastDerivative === 0 && derivative !== 0) {
                    addIntersection(currX, currY, currZ, 0, startPoint, endPoint, edgeStartTextureCoord, edgeEndTextureCoord)
                }
        ...
        function interpolationPrepareScanline(startIntersection, endIntersection, texture) {
            ...
            interpolationData.dz = (zEndFill - zStartFill) / deltaX;
            ...
            if (texture != null) {
                ...
                for (let i = 0; i <= 1; i++) {
                    ...
                    let textureDeltaFirstEdge = startIntersection.edgeEndTextureCoord[i] - startIntersection.edgeStartTextureCoord[i]
                    let texturePositionFirstEdge = startIntersection.edgeStartTextureCoord[i] + textureDeltaFirstEdge * interpolationWeightStart;

                    // Interpolate on second edge.
                    let textureDeltaSecondEdge = endIntersection.edgeEndTextureCoord[i] - endIntersection.edgeStartTextureCoord[i]
                    let texturePositionSecondEdge = endIntersection.edgeStartTextureCoord[i] + textureDeltaSecondEdge * interpolationWeightEnd

                    // Delta on scanline.
                    interpolationData.uvVecDelta[i] = (texturePositionSecondEdge - texturePositionFirstEdge) / deltaX;

                    // Starting value on scanline.
                    interpolationData.uvVec[i] = texturePositionFirstEdge
        ...
        function scanlineFillPolygon(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
            ...
            for (let y = 0; y < height; y++) {
                ...
                if ((line.length < 2) || (line.length % 2)) {
                    console.warn("Error in number of intersection (" + line.length + ") in line: " + y);
                }
                ...
                for (let p = 0; p < line.length; p += 2) {
                    ...
                    interpolationPrepareScanline(line[p], line[p + 1], texture);

                    // Fill line section inside polygon, loop x.
                    for (let x = line[p].x; x <= line[p + 1].x; x++) {

                        // Set z shorthand.
                        let z = interpolationData.z;
                        ...
                        if (zTest && horizontalClippingTest) {
                            ...
                            //
                            framebuffer.set(x, y, z, color, false, false) // without z-Test and dirty rectangle adjust.
                        }

                        // drawLineBresenham(startX, y, getZ(startX, y), endX, y, getZ(endX, y), color)
